    ...
    <div class="swiper">
      <div class="dots">
        <div v-if="electronicImageSwiperData.length > 1">
          <span>{{topSwiperIndex+1}}</span><span style="color: rgba(255,255,255,0.65)">/{{electronicImageSwiperData.length}}</span>
        </div>
      </div>
      
      <swiper style="height: 100%" :indicator-dots="false" :current="topSwiperIndex" @change="topSwiperTab">
        <swiper-item v-for="(item,index) in electronicImageSwiperData" :key="index">
          <div class="swiper-item">
            <img :src="item.imageUrl" alt="">
            <div class="itemPic">{{imageTypeFormat(item.imageType) || '-'}}</div>
          </div>
        </swiper-item>
      </swiper>
    </div>
  ...
  export default {
    ...
    methods:{
      ...
      getElectronicImageList(patientId, registerId) {
        customerAPI.healthRecordsImageList({ patientId: patientId, registerId: registerId }).then((res) => {
          this.electronicImageSwiperData = res.data
          console.log('this.electronicImageSwiperData',this.electronicImageSwiperData)
        })
      },
      topSwiperTab(e) {
        let that = this;
      ...
      onSavePicture () {
        ...
        console.log('thumbs[this.topSwiperIndex].url',thumbs[this.topSwiperIndex].imageUrl)
          wx.getImageInfo({
