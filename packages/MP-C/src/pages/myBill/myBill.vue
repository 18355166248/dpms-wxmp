<template>
  <div>
    <div v-if="myBillData.length !== 0" class="list" v-for="item in myBillData" @click="onHandleClick(item)">
      <div class="flexContent">
        <div class="leftText"><span class="icon iconfont icontime-circle colorFont" ></span><span class="colorFont" style="margin-left: 14rpx">{{ getUpdateTime(item.lastUpdateTime)}}</span></div>
        <div class="rightText"><span class="colorFont">{{ item.medicalInstitutionName }}</span></div>
      </div>
      <div class="lineHr"></div>
      <div class="flexContent">
        <div class="leftText"><span class="colorFont9">账单号：{{ item.billSerialNo }}</span></div>
        <div class="rightText"><span class="colorFont9">{{ billTypeFormat(item.billType)}}</span></div>
      </div>
      <div class="flexContent">
        <div class="leftText">{{ item.patientName }}</div>
        <div class="rightText" @click="onHandleClick(item.id)">
          <span>应收金额：</span><span style="font-size: 34rpx">¥{{ item.receivableAmount || 0}}</span>
        </div>
      </div>
      <div class="flexContent">
        <div class="leftText">
          <div v-if="item.arrearageAmount !== undefined">
            <span>欠费：</span><span style="color: #5cbb89;font-size: 34rpx">¥{{ item.arrearageAmount }}</span>
          </div>
        </div>
        <div class="rightText"><span>实付金额：</span><span style="color: #fa5151;font-size: 34rpx">¥{{ item.receiptAmount || 0}}</span></div>
      </div>
    </div>
    <load-more
        :status="loadStatus"
