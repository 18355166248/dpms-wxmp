  ...
  <view class="wyb-table-box">
    <view
      ...
      :style="{
        ...
        borderRight: showLeftAndRightBorder
    ...
    >
      ...
      >
        ...
        >
          <text
            :style="{
              marginLeft:
                autoSortShow(index) && textAlign !== 'left'
                  ? fontSize[0] * 0.65 + 'rpx'
                  : 0,
            }"
          >
      ...
      <view class="wyb-table-content">
        ...
        <view
          ...
          :style="{
            borderTop: cIndex === 0 ? 'none' : '1px solid' + borderColor,
          }"
        >
          <view
            class="wyb-table-content-item"
        ...
        if (this.formatCol.length !== 0) {
          this.formatCol.forEach((item) => {
            if (item.bottomComputedFormat) {
              if (header.key === item.key) {
                let needRplace = new RegExp(`\#${item['key']}\#`, 'mg')
                result = item.template.replace(
                  needRplace,
                  bottomComputed[item.key],
                )
              }
            }
          })
    ...
    doSort(key, type, isNumber) {
      ...
      if (type === 'asc') {
        ...
        if (isNumber) {
          ...
          arr.sort((a, b) => {
            let A = Pinyin.getSpell(a[key].charAt(0), function (
              charactor,
              spell,
            ) {
              return spell[1]
    ...
    onHeaderItemTap(index) {
      ...
      if (this.sortCol.length !== 0) {
        if (this.sortKeys.includes(header.key)) {
          ...
          let isNumber = this.sortCol[this.sortKeys.indexOf(header.key)]
            .isNumber
