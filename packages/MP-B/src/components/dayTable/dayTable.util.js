...
const scheduleTableUtil = {
          ...
          if (!noContainKeys.includes(dataItemKey)) {
            ...
            colList.forEach((col, colIndex) => {
              ...
              const {
                startTimestamp,
                endTimestamp
              } = rowDispatcher ? rowDispatcher(col) : col;
      ...
      .forEach(arrAreaKey => {
        ...
        arrArea.forEach(area => {
          ...
          area.forEach(areaItem => {
            ...
            data[dataIndex][parentKey].list[colIndex].$config = $config
          });
        });
      });
  ...
  calcCardPosition(config) {
    ...
    const baseOffset = rowStart === relativeCardVerticalStart ? 0 : (cardVerticalStart - relativeCardVerticalStart);
    ...
    const left =
      `calc(${100 * colOffset / colUnit}% + ${horizontalPadding - colOffset * horizontalPadding / colUnit}px)`;
