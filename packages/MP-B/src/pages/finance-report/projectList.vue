...
export default {
  ...
  onLoad() {
    ...
    this.getProject().then(() => {
      this.checked = this.parentChargeType.parentChargeTypeIds.split(',').map(item => Number(item));
    }).finally(() => {
      uni.hideLoading()
    });
  },
  ...
  methods: {
    ...
    onSave() {
      const value = {
        ...
        parentChargeTypeNames: this.list.filter(item => this.checked.includes(item.settingsChargeTypeId)).map(item => item.settingsChargeTypeName).join(','),
      };
