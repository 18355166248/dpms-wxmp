...
export default {
  ...
  onLoad({ name }) {
    ...
    this.loadData().then(() => {
      if(name === 'doctor') {
        this.checked=this.doctor.doctorIds.split(',').map(item => Number(item))
      } else if(name === 'consultant') {
        this.checked=this.consultant.consultantIds.split(',').map(item => Number(item))
      }
    }).finally(() => {
      uni.hideLoading()
    })
  ...
  methods: {
    ...
    loadData() {
      ...
      return institutionAPI.getStaffListByPositionFromAllInstitution({
        workStatus: this.$utils.getEnums('StaffStatus')?.STAFF_STATUS_AT_WORK_NAME?.value || 1,
        position,
      }).then(res => {
        console.log('res', res);
        this.list = res.data
    ...
    onSave() {
      ...
      const names = this.list.filter(item => this.checked.includes(item.staffId)).map(item => item.staffName).join(',')
      if(type === 'doctor') {
        value.doctorIds = ids
