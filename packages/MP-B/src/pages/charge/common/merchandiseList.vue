<template>
  <view class="list-wrap">
    <scroll-view scroll-y="true" class="scroll-wrap">
      <view class="item" v-for="item in merchandiseList" :key="item.merchandiseId">
        ...
        <view class="bottom-content">
          <view class="flex">
            ...
            <view class="infos">
              <span>可用库存：</span>
              <view>{{item.availableNum||0}}{{item.inventoryUnitStr}}</view>
            </view>
          </view>
          <view :style="{'color': '#fa5151'}">{{item.retailAmount | thousandFormatter(2, '￥')}}</view>
        </view>
      </view>
    </scroll-view>
  </view>
